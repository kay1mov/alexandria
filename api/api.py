...
class Auth:
    ...
    def __init__(self,
        ...
        self.available_homework_types = [
            "extra_task",
            "homework"
        ]

        self.available_homework_subtypes = [
            "reading",
            "listening",
            "grammar",
            "vocabulary",
    ...
    def exchange_points_to_coins(self, points: int, url: str = "https://student-api.cambridgeonline.uz/v1/student/change_points"):


        data = {
            "points": points,
        }
        response = requests.post(url, headers=self.build_headers("student-api.cambridgeonline.uz"), json=data)
    ...
    def get_homework(self, homework_type, homework_subtype, homework_info):
        ...
        if homework_type.lower() not in self.available_homework_types:
            print(f"Error in homework_type: {homework_type}")
            return None

        if homework_subtype.lower() not in self.available_homework_subtypes:
